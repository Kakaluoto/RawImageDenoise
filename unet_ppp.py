...
class UNet_3Plus_s(nn.Module):

    def __init__(self, in_channels=4, out_channels=4, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus_s, self).__init__()
        ...
        # output
        self.outconv1 = nn.Conv2d(self.UpChannels, out_channels, kernel_size=1, stride=1)
    ...
    def forward(self, inputs):
        ...
        inputs = F.pad(inputs, (0, w_pad, 0, h_pad), 'replicate')
        ## -------------Encoder-------------
...
if __name__ == "__main__":
    ...
    test_input = torch.from_numpy(np.random.randn(1, 4, 1736, 2312)).float()
    test_input = test_input.cuda()
    # net = UNet_3Plus_s().cuda()
    net = UNet_3Plus().cuda()
    output = net(test_input)
    torch.save(net.state_dict(), "./checkpoints/unet_ppp/test.pth")
    print("test over")
