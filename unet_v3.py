# coding=utf-8
import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np


class Unet_v3(nn.Module):
    def __init__(self, in_channels=4, out_channels=4):
        super(Unet_v3, self).__init__()

        # device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.conv1_1 = nn.Conv2d(in_channels, 32, kernel_size=3, stride=1, padding=1)
        self.conv1_2 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2)
        # self.pool1 = nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1)

        self.conv2_1 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        self.conv2_2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)
        self.pool2 = nn.MaxPool2d(kernel_size=2)
        # self.pool2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1)

        self.conv3_1 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)
        self.conv3_2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)
        self.pool3 = nn.MaxPool2d(kernel_size=2)
        # self.pool3 = nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1)

        self.conv4_1 = nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1)
        self.conv4_2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
        self.pool4 = nn.MaxPool2d(kernel_size=2)
        # self.pool4 = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1)

        self.conv5_1 = nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1)
        self.conv5_2 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1)

        self.upv6 = nn.ConvTranspose2d(512, 256, 2, stride=2)
        self.conv6_1 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
        self.conv6_2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)

        self.upv7 = nn.ConvTranspose2d(256, 128, 2, stride=2)
        self.conv7_1 = nn.Conv2d(256 + 128, 128, kernel_size=3, stride=1, padding=1)
        self.conv7_2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)

        self.upv8 = nn.ConvTranspose2d(128, 64, 2, stride=2)
        self.conv8_1 = nn.Conv2d(128 + 64, 64, kernel_size=3, stride=1, padding=1)
        self.conv8_2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)

        self.upv9 = nn.ConvTranspose2d(64, 32, 2, stride=2)
        self.conv9_1 = nn.Conv2d(64 + 32, 32, kernel_size=3, stride=1, padding=1)
        self.conv9_2 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)

        self.conv10_1 = nn.Conv2d(32, out_channels, kernel_size=1, stride=1)

        self.upv11 = nn.ConvTranspose2d(256, 128, 2, stride=2)  # 来自conv4的上采样
        self.conv11_1 = nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1)
        self.conv11_2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1)

        self.upv12 = nn.ConvTranspose2d(128, 64, 2, stride=2)  # 来自conv4的上采样的上采样
        self.conv12_1 = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1)
        self.conv12_2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)

        self.upv13 = nn.ConvTranspose2d(64, 32, 2, stride=2)  # 来自conv4的上采样的上采样
        self.conv13_1 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
        self.conv13_2 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)

        self.leaky_relu = nn.LeakyReLU(0.2)

    def forward(self, x):
        n, c, h, w = x.shape
        h_pad = 32 - h % 32 if not h % 32 == 0 else 0
        w_pad = 32 - w % 32 if not w % 32 == 0 else 0
        padded_image = F.pad(x, (0, w_pad, 0, h_pad), 'replicate')

        conv1 = self.leaky_relu(self.conv1_1(padded_image))
        conv1 = self.leaky_relu(self.conv1_2(conv1))
        pool1 = self.pool1(conv1)

        conv2 = self.leaky_relu(self.conv2_1(pool1))
        conv2 = self.leaky_relu(self.conv2_2(conv2))
        pool2 = self.pool2(conv2)

        conv3 = self.leaky_relu(self.conv3_1(pool2))
        conv3 = self.leaky_relu(self.conv3_2(conv3))
        pool3 = self.pool3(conv3)

        conv4 = self.leaky_relu(self.conv4_1(pool3))
        conv4 = self.leaky_relu(self.conv4_2(conv4))
        pool4 = self.pool4(conv4)

        conv5 = self.leaky_relu(self.conv5_1(pool4))
        conv5 = self.leaky_relu(self.conv5_2(conv5))

        up6 = self.upv6(conv5)
        up6 = torch.cat([up6, conv4], 1)
        conv6 = self.leaky_relu(self.conv6_1(up6))
        conv6 = self.leaky_relu(self.conv6_2(conv6))

        up11 = self.upv11(conv4)
        up11 = torch.cat([up11, conv3], 1)
        conv11 = self.leaky_relu(self.conv11_1(up11))
        conv11 = self.leaky_relu(self.conv11_2(conv11))

        up7 = self.upv7(conv6)
        up7 = torch.cat([up7, conv3, conv11], 1)
        conv7 = self.leaky_relu(self.conv7_1(up7))
        conv7 = self.leaky_relu(self.conv7_2(conv7))

        up12 = self.upv12(conv11)
        up12 = torch.cat([up12, conv2], 1)
        conv12 = self.leaky_relu(self.conv12_1(up12))
        conv12 = self.leaky_relu(self.conv12_2(conv12))

        up8 = self.upv8(conv7)
        up8 = torch.cat([up8, conv2, conv12], 1)
        conv8 = self.leaky_relu(self.conv8_1(up8))
        conv8 = self.leaky_relu(self.conv8_2(conv8))

        up13 = self.upv13(conv12)
        up13 = torch.cat([up13, conv1], 1)
        conv13 = self.leaky_relu(self.conv13_1(up13))
        conv13 = self.leaky_relu(self.conv13_2(conv13))

        up9 = self.upv9(conv8)
        up9 = torch.cat([up9, conv1, conv13], 1)
        conv9 = self.leaky_relu(self.conv9_1(up9))
        conv9 = self.leaky_relu(self.conv9_2(conv9))

        conv10 = self.conv10_1(conv9) + self.conv10_1(conv13)
        out = conv10[:, :, :h, :w]

        return out


if __name__ == "__main__":
    test_input = torch.from_numpy(np.random.randn(1, 4, 512, 512)).float()
    # test_input = torch.from_numpy(np.random.randn(1, 4, 2312, 2312)).float()
    # test_input = torch.from_numpy(np.random.randn(1, 4, 1736, 1736)).float()
    # test_input = torch.from_numpy(np.random.randn(1, 4, 512, 512)).float()
    net = Unet_v3()
    output = net(test_input)
    torch.save(net.state_dict(), "./checkpoints/unet_v3/test.pth")
    print("test over")
